...
export default function App() {
  ...
  return (
    <main className="flex h-screen select-none relative">
      <aside className="w-1/5 h-full p-2 border-r overflow-auto">
        ...
        <div className="bg-blue-900 space-y-3 p-2 rounded-b-lg">
          <SelectPdf
            value={selPdf}
            onChange={url => { setSelPdf(url); setHighlight(''); }}
          />
          <SearchRamal
            data={ramais}
            inactive={inactive}
            onSelect={({ andar, comment }) => {
      ...
      <section className="w-4/5 h-full overflow-auto p-4 bg-gray-50">
        {pages.length > 0 ? (
          <PdfViewer
            pages={pages}
            boxes={boxes}
      ...
      <img
        src="/VSlogo_small.png"
        alt="VS Logo"
        className="absolute bottom-4 right-4 w-12 h-auto opacity-100"
      />
    </main>
  );
}