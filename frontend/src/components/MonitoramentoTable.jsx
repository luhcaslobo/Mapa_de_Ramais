...
export default function MonitoramentoTable({ data, ramais, onSelect }) {
  ...
  const nomeByRamal = new Map(
    subscribers.map(s => [String(s.Directory_Number), s.Annu_Name ?? ''])
  );
  const andarByRamal = new Map(
    subscribers.map(s => [String(s.Directory_Number), s.Annu_First_Name ?? ''])
  );
  const portaByRamal = new Map(
    subscribers.map(s => [String(s.Directory_Number), s.UTF8_Comment3 ?? ''])
  );

  const sorted = [...data].sort(
    (a, b) => b['horário'].localeCompare(a['horário'])
  );
  ...
  const handleContextMenu = (e, dn) => {
    ...
    setMenu({ visible: true, x: e.clientX, y: e.clientY, dn, editing: false });
  };
  ...
  return (
    <div className=" max-h-screen text-xs">
      <h1 className="
        text-white font-bold text-center uppercase text-sm mb-3 rounded-lg bg-blue-900 py-1
      ">
      ...
      {sorted.map(({ 'dir nb': dn, horário }) => {
        ...
        return (
          <div
            key={dn}
            className="mb-2 p-2 border rounded shadow-sm cursor-pointer hover:bg-gray-300 whitespace-nowrap"
          ...
          >
            <div className="font-semibold">Ramal: {dn} - {nomeByRamal.get(String(dn)) || '—'}</div>
            <div>Andar: {andarByRamal.get(String(dn)) || '—'} - Porta: {portaByRamal.get(String(dn)) || '—'}</div>
      ...
      {menu.visible && (
        ...
        >
          {menu.editing ? (
            ...
            <button
              onClick={e => { e.stopPropagation(); setMenu(prev => ({ ...prev, editing: true })); }}
              className="px-3 py-1 bg-blue-700 text-white rounded hover:bg-blue-600"
            >Editar Anotação</button>
          )}
        </div>
      )}
