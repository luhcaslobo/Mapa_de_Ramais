...
export default function PdfPage({
  ...
  return (
    <div
      ...
      style={{ width: "100%", maxHeight: isMobile ? "80vh" : "95vh" }}
    >
      <div
        ...
        role="presentation"
        className="relative border border-gray-300 rounded shadow cursor-grab touch-none"
        style={{
          width: page.w,
          height: page.h,
      ...
      >
        <img src={page.src || "/placeholder.svg"} alt="" className="pointer-events-none w-full h-full" />

        {boxes
          .filter((b) => (b.page || 1) === page.page)
          .map((b, idx) => {
            const style = {
              ...toCanvas(b, page),
              position: "absolute",
              ...
              zIndex: highlight === b.texto ? 10 : topIdx === idx ? 5 : 1,
            }
            const lista = commentMap[b.texto] || []
            const listaFiltered = lista.filter(
              (ramal) =>
                filterStatus === "todos" ||
            ...
            return (
              <span
                key={idx}
                onMouseEnter={() => {
                  if (showAll) setTopIdx(idx)
                  else setHovered({ ...style, lista: listaFiltered, page: page.page })
                }}
                onMouseLeave={() => {
                  if (!showAll) setHovered(null)
                }}
                onTouchStart={(e) => {
                  e.stopPropagation()
                  if (showAll) setTopIdx(idx)
